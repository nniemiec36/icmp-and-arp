...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        imcp_header = recPacket[20:28] # 8 bytes is the length of an ICMP Header
        imcp_header = struct.unpack("bbHHh", imcp_header)
        ...
        if timeLeft - howLongInSelect > 0:
            ...
            if(rtt_time < 1):
                rtt_time = rtt_time #'<1' #For display purposes. 
            else:
                rtt_time = '{:.0f}'.format(rtt_time) #Also for display purposes. 
            timeLeft = '{:.0f}'.format(timeLeft * 1000)
            return ('{} bytes from {}; time = {} ms'.format(packetLength, destAddr, rtt_time))
...
def ping(host, timeout=1):
    ...
    try:
       ...
       if cnt != 0:
            ...
            if rtt_cnt != 0:
                print('Approximate statistics in milli-seconds:')
                print ('\tMinimum round-trip = {:.0f}ms, Maximum round-trip = {:.0f}ms, Average round-trip = {:.0f}ms'.format(rtt_min, rtt_max, rtt_sum / rtt_cnt))
        # TODO END
